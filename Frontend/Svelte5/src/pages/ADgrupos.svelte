...
let users = $state([]); // Usuarios del sistema
...
async function fetchAllRecords() {
  try {
    const res = await api.get(`/groupAdmin`);
...
async function fetchUsers() {
  ...
  try {
    const res = await api.get(`/user`);   //recogemos los ususarios del grupo
    // console.log('Respuesta de la API:', res);
    // El array real está aquí:
    const array = res.data.data;

    const recordsNormalized = array.map(({ id_user: id, name_surname: label,  ...rest }) => ({
        id,
        label
    }));
    users = recordsNormalized;
    // console.log("Usuarios del grupo cargados:", users);
...
async function handleEditorAction({ item, values }) {
  ...
  if (item.id === "save") {
    if (editorMode === "add") {
        ...
        api.post(`/groupAdmin`, payload) // Añadimos grupo
        .then(res => {
        ...
        api.put(`/groupAdmin/${payload.id}`, payload) // Editamos grupo
        .then(res => {
...
try {
    await api.delete(`/groupAdmin/${itemToDelete.id}`);
...
<div class="d-flex justify-content-between align-items-center mb-3">
  <h3><Fa icon={faPeopleGroup} /> Gestión de Grupos</h3>
...
{:else}
    ...
    >
        ...
        <div style="height: 560px; max-width: 500px;">
        <Grid
            bind:this={gridRef}
