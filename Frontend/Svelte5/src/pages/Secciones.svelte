...
{ key: "section", label: "Sección", comp: "text", required: true, maxLength: 30 }
]);
...
try {
    const res = await api.get(`/secctionGroup/${groupSelected.id}`);
    // console.log('Respuesta de la API:', res);
    // El array real está aquí:
    const array = res.data.data;
    // Normalizamos id_section_group → id
    const recordsNormalized = array.map(({ id_section_group: id, ...rest }) => ({
        id,
        ...rest
    }));
...
try {
    if (item.id === "save") {
    if (editorMode === "add") {
        const res = await api.post(`/secctionGroup/${groupSelected.id}`, payload);
        ...
        await api.put(`/secctionGroup/${groupSelected.id}/${values.id}`, payload);
        addToast({
...
try {
    await api.delete(`/secctionGroup/${groupSelected.id}/${itemToDelete.id}`);
    ...
    addToast({
    message: "No se pudo eliminar el sección.",
    type: 'error',
    dismissible: true,
    timeout: 0
    });
...
<div class="d-flex justify-content-between align-items-center mb-3">
  <h3><Fa icon={faLayerGroup} /> Gestión de Secciones del Grupo: "{groupSelected?.name}"</h3>
...
{#if showDeleteConfirm}
  <ModalArea>
    <div class="modal-content">
      ...
      <p>Sección: <b>{itemToDelete?.section}</b></p>
      <div class="actions">
