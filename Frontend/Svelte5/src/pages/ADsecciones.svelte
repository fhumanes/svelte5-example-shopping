...
try {
    const res = await api.get(`/sectionTemplate`);
    // console.log('Respuesta de la API:', res);
    // El array real está aquí:
    const array = res.data.data;
    // Normalizamos id_section_group → id
    const recordsNormalized = array.map(({ id_section_template: id, ...rest }) => ({
        id,
        ...rest
    }));
...
try {
    if (item.id === "save") {
    if (editorMode === "add") {
        const res = await api.post(`/sectionTemplate`, payload);
        ...
        await api.put(`/sectionTemplate/${values.id}`, payload);
        addToast({
...
try {
    await api.delete(`/sectionTemplate/${itemToDelete.id}`);
...
<div class="d-flex justify-content-between align-items-center mb-3">
  <h3><Fa icon={faLayerGroup} /> Gestión de la Plantilla de Secciones</h3>
